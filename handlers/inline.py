...
async def handle_send_request(callback: CallbackQuery):
    try:
        ...
        logger.info(f"Пользователь {user_id} запросил отправку письма для сделки {trade_id}")
        ...
        logger.info(f"Данные сделки {trade_id}: {trade_data}")
        if not trade_data:
        ...
        # Всегда оставляем кнопки "Составить заявку" и "Добавить в избранное"
        ...
        if success:
            # Редактируем сообщение: оставляем кнопки + добавляем уведомление
            ...
            # Ошибка — тоже оставляем кнопки, но с предупреждением
            await callback.message.edit_text(
                f"{callback.message.html_text}\n\n"
